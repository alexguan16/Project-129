...
void* memSearchAVX2(uint8_t* haystack, size_t hSz, uint8_t* needle, size_t nSz) {
	...
	for(; i <= check; i += 0x20) {
		__m256i vec2 = _mm256_load_si256((__m256i const*)(haystack + i + 0x20));
		__m256i comb = _mm256_permute2x128_si256(vec1, vec2, 0x21);
		__m256i shft = _mm256_alignr_epi8(comb, vec1, 1);

		__m256i m1 = _mm256_cmpeq_epi8(vec1, b1);
